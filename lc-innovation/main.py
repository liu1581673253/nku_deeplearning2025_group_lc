...
def main():
    ...
    scheduler = StepLR(optimizer, step_size=50, gamma=5e-5 / 7e-5)
    bce_fn = nn.BCELoss()

    epochs = 50
    edge_loss_weight = 0.65  # λ 边缘损失权重
    ...
    output_fig_path = "./training_loss_curve.png"
    os.makedirs(os.path.dirname(output_fig_path), exist_ok=True)
    plt.savefig(output_fig_path)
    print(f"训练损失曲线已保存到 {output_fig_path}")
    plt.show()
    ...
    output_fig_path = "./test_mae_curve.png"
    os.makedirs(os.path.dirname(output_fig_path), exist_ok=True)
    plt.savefig(output_fig_path)
    print(f"测试MAE曲线已保存到 {output_fig_path}")
    plt.show()
    ...
    plt.ylabel("MaxF")
    plt.legend()
    plt.title("Test MaxF Curve")
    output_fig_path = "./test_maxf_curve.png"
    os.makedirs(os.path.dirname(output_fig_path), exist_ok=True)
    plt.savefig(output_fig_path)
    print(f"测试MaxF曲线已保存到 {output_fig_path}")
    plt.show()
